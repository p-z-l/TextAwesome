...
class Utils {
	static func findNSRangeOfPattern(string: String, pattern: String, caseSensitive: Bool)
		-> [NSRange]
	{
	...
	static func findNSRangeOfMultiplePatterns(
		string: String, patterns: [String], caseSensitive: Bool
	) -> [NSRange] {
		var result = [NSRange]()
	...
	static func attributeMatchingResults(
		...
		let searchResults = Utils.findNSRangeOfPattern(
			string: attributedString.string, pattern: pattern, caseSensitive: caseSensitive)
